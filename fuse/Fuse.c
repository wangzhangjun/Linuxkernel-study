...
/*
gcc Fuse.c -o myFuse `pkg-coig fuse3 --cflags --libs`
测试：
gcc Fuse.c -o myFuse `pkg-config fuse3 --cflags --libs`
mkdir alert
./myFuse -f alert
新开一个窗口，然后cat alert/alertMe , echo "fadsf" > alert/alertMe 
观察其输出，会发现fuse_read ,fuse_write等被调用到了。
*/
...
int fun_readdir( const char *path, void *buff, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi ) {
	filler(buff, ".", NULL, 0, FUSE_FILL_DIR_PLUS); // Current Directory
	filler(buff, "..", NULL, 0, FUSE_FILL_DIR_PLUS); // Parent Directory

	// Get the files/directories which are exist in the mount point
	if ( strcmp( path, "/" ) == 0 )
		filler(buff, _fileName, NULL, 0, FUSE_FILL_DIR_PLUS);
	else 
		return -ENOENT;
...
{
	printf("\033[47;31mfuse open.\n\033[0m");
	struct fuse_context *context = fuse_get_context();
	printf("in open pid:%d\n", context->pid);
	printf("in open uid:%d\n", context->uid);
	printf("in open gid:%d\n", context->gid);
	printf("in open file path:%s\n", path);
...
int fun_read( const char *path, char *buff, size_t size, off_t offset, struct fuse_file_info *fi ) {
	...
	printf("\033[47;31mfuse read.\n\033[0m");
	struct fuse_context *context = fuse_get_context();
	printf("in read pid:%d\n", context->pid);
	printf("in read uid:%d\n", context->uid);
	printf("in read gid:%d\n", context->gid);
	printf("in read file path:%s\n", path);
...
int fun_write(const char *path, const char *buff, size_t size, off_t offset, struct fuse_file_info *fi) {
	printf("\033[47;31mfuse write.\n\033[0m");
	struct fuse_context *context = fuse_get_context();
	printf("in write pid:%d\n", context->pid);
	printf("in write uid:%d\n", context->uid);
	printf("in write gid:%d\n", context->gid);
	printf("in write file path:%s\n", path);
	if (strcmp( path, fileName ) == 0 )
	{		
